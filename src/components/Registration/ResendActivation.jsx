...
const ResendActivation = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setErrorMsg("");
    try {
      await apiClient.post("/auth/users/resend_activation/", { email: data.email });
      setSuccessMsg("Activation email has been sent successfully.");
    } catch (error) {
      setErrorMsg(
        error?.response?.data?.detail || "Failed to send activation email."
      );
    }
  };
